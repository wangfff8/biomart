...
ensembl_datasets_file = os.path.join(script_path, "doc", "biomart_genes_datasets.json")
...
my_colnames = [
    ...
    'Gene name', 'Gene description', 'NCBI gene (formerly Entrezgene) ID', 
    'GO term accession', 'Gene type', 'Chromosome/scaffold name', 'Gene start (bp)', 
    'Gene end (bp)','Transcript length (including UTRs and CDS)', 'Strand'
]
...
def get_attrs(level):
    options = {
        ...
        'transcript_description': ['ensembl_transcript_id', 'ensembl_gene_id', 
...
def get_virtual_schema(species_type):
    virtual_schema_names = {
        'animal': 'default',
        'plant': 'plants_mart',
        'fungi': 'fungi_mart'
    }

    return virtual_schema_names[species_type]
...
if __name__ == '__main__':
    ...
    my_proxy = {'http_proxy': 'http://172.29.48.1:31181'}
    ...
    virtual_schema = get_virtual_schema(species_type)
    ...
    qry_attributes = {"attributes": my_attributes_test}
    params.update(qry_attributes)
